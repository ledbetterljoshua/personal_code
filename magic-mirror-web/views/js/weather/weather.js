var weather = {
	...
	iconTable: {
		...
		'50n':'wi wi-night-alt-cloudy-windy'
	},
...
weather.updateCurrentWeather = function () {
	...
	$.ajax({
		...
		success: function (data) {
			...
			var _temperature = this.roundValue(data.main.temp * 1.8 + 32),
				_temperatureMin = this.roundValue(data.main.temp_min * 1.8 + 32),
				_temperatureMax = this.roundValue(data.main.temp_max * 1.8 + 32),
	...
	$.ajax({
		...
		success: function (data) {
			...
			for (var i = 0, count = data.list.length; i < count; i++) {
				...
				_forecastHtml2 += '<td class="icon-small ' + this.iconTable[_forecast.weather[0].icon] + '"></td>';
				_forecastHtml3 += '<td class="temp-max">' + this.roundValue(_forecast.temp.max) + '</td>';
				_forecastHtml4 += '<td class="temp-min">' + this.roundValue(_forecast.temp.min) + '</td>';
